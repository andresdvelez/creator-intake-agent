...
export function WorkflowSteps({ hasAiReview, isReviewing, currentStatus }: WorkflowStepsProps) {
  const s1: StepState = 'done'
  const s2: StepState = isReviewing ? 'active' : hasAiReview ? 'done' : 'pending'
  const s3: StepState = currentStatus !== 'pending' ? 'done' : hasAiReview ? 'active' : 'pending'
  ...
  return (
    <div className="flex items-center gap-1.5 mt-1.5">
      ...
      <div className={`h-px w-5 ${CONNECTOR[s2 === 'pending' ? 'dim' : 'lit']}`} />

      <div className={`h-1.5 w-1.5 rounded-full shrink-0 ${DOT[s2]}`} />
      <span className={`text-[10px] ${LABEL[s2]}`}>AI Review</span>

      <div className={`h-px w-5 ${CONNECTOR[s3 === 'pending' ? 'dim' : 'lit']}`} />
