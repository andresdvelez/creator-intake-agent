...
describe('AiReviewCard', () => {
  ...
  it('shows "Reject" badge for reject recommendation', () => {
    render(<AiReviewCard review={{ ...baseReview, recommendation: 'reject' }} />)
    expect(screen.getByText('Reject')).toBeInTheDocument()
  })

  it('shows "Needs Info" badge for needs_info recommendation', () => {
    render(<AiReviewCard review={{ ...baseReview, recommendation: 'needs_info' }} />)
    expect(screen.getByText('Needs Info')).toBeInTheDocument()
  })

  it('shows "Manual Review" badge for manual_review recommendation', () => {
    render(<AiReviewCard review={{ ...baseReview, recommendation: 'manual_review' }} />)
  ...
  it('renders risks items', () => {
    render(<AiReviewCard review={baseReview} />)
    expect(screen.getByText('Potential overlap with competitor brands')).toBeInTheDocument()
  })
  ...
  it('renders the suggestedReply', () => {
    render(<AiReviewCard review={baseReview} />)
    expect(
      screen.getByText('Thank you for applying! We would love to work with you.')
    ).toBeInTheDocument()
  })
