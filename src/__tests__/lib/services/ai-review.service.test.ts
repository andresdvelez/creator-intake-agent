...
import { parseAndValidate, runAiReview } from '@/lib/services/ai-review.service'
...
describe('parseAndValidate', () => {
  ...
  it('returns error when JSON does not match schema', () => {
    const result = parseAndValidate(JSON.stringify({ fitScore: 'high', recommendation: 'maybe' }))
    expect(result.success).toBe(false)
    if (!result.success) {
